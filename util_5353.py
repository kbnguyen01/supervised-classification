...
def assert_int_range(int_range, v, prob):
  assert_int(v, prob)
  assert_int(int_range[0], prob)
  assert_int(int_range[1], prob)
...
def assert_str(v, prob, valid_values=None):
  if type(v) != str and type(v) != numpy.str_:
...
def assert_tuple(v, tup_len, prob):
  ...
  if len(v) != tup_len:
    die(prob, 'Tuple len should be %d, found %d', (tup_len, len(v)))
...
def assert_set(v, prob, valid_values=None):
  if type(v) != set:
    die(prob, 'Value should be a set, found %s', type(v))
  if valid_values != None:
    for item in v:
      if item not in valid_values:
        die(prob, 'Invalid set item: %s   Valid: %s', (item, valid_values))
